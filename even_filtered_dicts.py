...
def main():
    ...
    mkdir(TMP_DIR)
    mean_len: int = ceil(total_len / N_DICTS)

    alpha_filtered_dir: str = iterate_filtered_dicts(
        alpha=alpha, func=add_save, use_p=True, mean_len=mean_len, local_vars=local_vars
    )

    next_dict: dict = local_vars[NEXT_DICT_KEY]

    assert len(next_dict) <= mean_len
    ...
    assert len(listdir(TMP_DIR)) == N_DICTS
    assert local_vars[IDX_KEY] == N_DICTS

    rmtree(alpha_filtered_dir)
    rename(src=TMP_DIR, dst=alpha_filtered_dir)


def add_save(feat1: str, feat2: str, p: float, mean_len: int, local_vars: dict):
